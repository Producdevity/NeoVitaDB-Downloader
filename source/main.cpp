...
int screenshots_feature = FEATURE_OFF;
int trailer_feature = FEATURE_OFF;
int trophies_feature = FEATURE_OFF;
...
void load_preview(AppSelection *game) {
	...
	if (mode_idx == MODE_THEMES) {
		...
		sprintf(banner_path, "ux0:data/NeoVitaDB/icons/%c%c/%s", game->icon[0], game->icon[1], game->icon);
...
void LoadScreenshot() {
	...
	char banner_path[256];
	sprintf(banner_path, "ux0:data/NeoVitaDB/ss%d.png", cur_ss_idx);
	int w, h;
...
int trophy_loader(unsigned int args, void *arg) {
	...
	while (t) {
		...
		int w, h;
		char fname[256];
		sprintf(fname, "ux0:data/NeoVitaDB/trophies/%s/%s", t->titleid, t->icon_name);
		GLuint res;
		glGenTextures(1, &res);
...
void PrepareTrophy(const char *tid, const char *name, int index, int count) {
	...
	sprintf(fname, "ux0:data/NeoVitaDB/trophies/%s/%s", tid, name);
	SceIoStat st;
...
int main(int argc, char *argv[]) {
	...
	sceIoMkdir("ux0:data/NeoVitaDB/trophies", 0777);
...
extract_libshacccg:
	...
	sceIoMkdir("ux0:data/NeoVitaDB/icons", 0777);
	if (sceIoGetstat("ux0:/data/NeoVitaDB/icons.db", &st) < 0) {
		SceUID idx = sceIoOpen("ux0:data/NeoVitaDB/icons.db", SCE_O_WRONLY | SCE_O_CREAT | SCE_O_TRUNC, 0777);
		sceIoClose(idx);
	}
	...
	if (strlen(boot_params) == 0) {
		if (!(pad.buttons & SCE_CTRL_RTRIGGER) || sceIoGetstat("ux0:data/NeoVitaDB/apps.json", &st) < 0) {
		...
		is_vitadb_online = populate_apps_database("ux0:data/NeoVitaDB/apps.json", false);
	} else {
		is_vitadb_online = populate_apps_database("ux0:data/NeoVitaDB.json", false);
	}
	...
	while (!update_detected) {
		...
		if (extra_menu_invoked) {
			...
			if (hovered->trophies) {
				if (ImGui::Button("View Available Trophies", ImVec2(-1.0f, 0.0f))) {
					trophies_feature = FEATURE_LOADING;
				}
			}
			if (hovered->requirements) {
			...
			if (strlen(hovered->trailer) > 5) {
				if (ImGui::Button("View Trailer", ImVec2(-1.0f, 0.0f))) {
					trailer_feature = FEATURE_LOADING;
				}
			}
			if (strlen(hovered->source_page) > 5) {
			...
			if (ImGui::Button("View Changelog", ImVec2(-1.0f, 0.0f))) {
				show_changelog = true;
				changelog = get_changelog("ux0:data/NeoVitaDB/apps.json", hovered->id);
			}
			ImGui::End();
		}
		
		if (screenshots_feature == FEATURE_ON) {
			LoadScreenshot();
		...
		if (show_changelog) {
			char titlebar[256];
			sprintf(titlebar, "%s Changelog (%s to close)", hovered->name, SCE_CTRL_CANCEL_LITERAL);
			ImGui::SetNextWindowPos(ImVec2(80, 55), ImGuiSetCond_Always);
			ImGui::SetNextWindowSize(ImVec2(800, 472), ImGuiSetCond_Always);
		...
		if (show_requirements) {
			char titlebar[256];
			sprintf(titlebar, "%s Requirements (%s to close)", hovered->name, SCE_CTRL_CANCEL_LITERAL);
			ImGui::SetNextWindowPos(ImVec2(80, 55), ImGuiSetCond_Always);
			ImGui::SetNextWindowSize(ImVec2(800, 472), ImGuiSetCond_Always);
			ImGui::Begin(titlebar, nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse);
		...
		if (trophies_feature == FEATURE_ON) {
			char titlebar[256];
			sprintf(titlebar, "%s Trophies (%s to close)", hovered->name, SCE_CTRL_CANCEL_LITERAL);
			ImGui::SetNextWindowPos(ImVec2(80, 55), ImGuiSetCond_Always);
			ImGui::SetNextWindowSize(ImVec2(800, 472), ImGuiSetCond_Always);
			ImGui::Begin(titlebar, nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse);
			ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(0.0f, 0.5f));
		...
		sceCtrlPeekBufferPositive(0, &pad, 1);
		if (pad.buttons & SCE_CTRL_LTRIGGER && !(oldpad & SCE_CTRL_LTRIGGER) && !trailer_feature && !screenshots_feature && !show_changelog && !show_requirements && !trophies_feature && !extra_menu_invoked) {
		...
		} else if (pad.buttons & SCE_CTRL_RIGHT && !(oldpad & SCE_CTRL_RIGHT) && !trailer_feature && !show_changelog && !show_requirements && !trophies_feature && (!extra_menu_invoked || screenshots_feature)) {
			if (screenshots_feature)
				cur_ss_idx++;
			else
				fast_increment = true;
		...
		#define setup_anti_burn_in() \
			if (!anti_burn_in_set_up) { \
				anti_burn_in_texture = capture_previous_frame(); \
				anti_burn_in_set_up = true; \
			}
...
skip_install:		
		...
		if (trophies_feature == FEATURE_LOADING) {
			setup_anti_burn_in();
			char dl_url[512];
			sprintf(dl_url, "ux0:data/NeoVitaDB/trophies/%s", hovered->titleid);
			...
			free(trp_data);
			trophies_feature = FEATURE_ON;
		}
		
		// Queued trailer request
		if (trailer_feature == FEATURE_LOADING) {
			if (has_animated_bg) {
				video_close();
			}
		...
		if (screenshots_feature == FEATURE_LOADING) {
			setup_anti_burn_in();
			sceIoRemove("ux0:data/NeoVitaDB/ss0.png");
			sceIoRemove("ux0:data/NeoVitaDB/ss1.png");
			sceIoRemove("ux0:data/NeoVitaDB/ss2.png");
			sceIoRemove("ux0:data/NeoVitaDB/ss3.png");
			old_ss_idx = -1;
			cur_ss_idx = 0;
			int shot_idx = 0;
			if (mode_idx == MODE_THEMES) {
				...
				sceIoRename(TEMP_DOWNLOAD_NAME, "ux0:data/NeoVitaDB/ss0.png");
			} else {
				...
				while (shot_idx < shot_num) {
					...
					sprintf(shot_links[shot_idx], "ux0:data/NeoVitaDB/ss%d.png", shot_idx);
					sceIoRename(TEMP_DOWNLOAD_NAME, shot_links[shot_idx++]);
				}
			}
			screenshots_feature = FEATURE_ON;
		}
		...
		if (mode_idx == MODE_PSP_HBS && !psp_apps) {
			...
			sceKernelWaitThreadEnd(thd, NULL, NULL);
			populate_apps_database("ux0:data/NeoVitaDB/psp_apps.json", true);
		}
	}
